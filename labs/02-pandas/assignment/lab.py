...
def med_score(scores):
    """
    returns the median score for those students who passed the test.
    :param scores: DataFrame from the second question
    :return: median score
    >>> scores_fp = os.path.join('data', 'scores.csv')
    >>> scores = data_load(scores_fp)
    >>> scores = pass_fail(scores)
    >>> med = med_score(scores)
...
def super_hero_powers(powers):
    ...
    powers and returns a list with the following three entries:
        ...
        - The name of the second most common superpower among superheroes who can fly 
          (the most common being "Flight" itself).
